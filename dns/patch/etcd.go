...
func (e *Etcd) Lookup(state request.Request, name string, typ uint16) (*dns.Msg, error) {
	fmt.Printf("1---name=[%v]--typ=[%v]-----state=[%v]\n", name, typ, state)
...
func (e *Etcd) Records(state request.Request, exact bool) ([]msg.Service, error) {
	...
	//check if in except
	if e.isInExcept(name) {
		return e.except(state, exact)
	}
	except, er := e.get(e.Except+"/"+name, true)		
	fmt.Printf("except=[%v], er = [%v]\n", except, er)
	fmt.Printf("e.Upstream=[%+v], type=%T\n", e.Upstream.Forward, e.Upstream.Forward)
	

	path, star := msg.PathWithWildcard(name, e.PathPrefix)
	r, err := e.get(path, true)
	fmt.Printf("name=[%v], path=[%v], r=%v\n", name, path, r)
	fmt.Printf("e = [%v]\n", err)
	if err != nil {
		return nil, err
	}
